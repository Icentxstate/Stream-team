...
latest_values = (filtered_df.sort_values("ActivityStartDate")
                 .groupby("StationKey").tail(1).set_index("StationKey"))
...
basemap_option = st.sidebar.selectbox("🗺️ Basemap Style", list(basemap_tiles.keys()))
...
if st.session_state.view == "map":
    ...
    if selected:
        ...
        stats_table = plot_df.describe().T
        st.dataframe(stats_table.style.format("{:.2f}"))
        csv_stats = stats_table.to_csv().encode("utf-8")
